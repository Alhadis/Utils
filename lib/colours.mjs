 ...
 * ];
 * @param {Number[]|BigInt[]} bitmap
 * @param {Number} [width=32]
 * @param {Number} [colour=0x000000FF]
 * @return {Uint8ClampedArray}
 */
export function bitmapToRGBA(bitmap, width = 32, colour = 0x000000FF){
	const cast = width > 32 ? BigInt : Number;
	const rows = bitmap.length;
	const rgba = new Uint8ClampedArray(width * rows * 4);
	for(let y = 0; y < rows; ++y){
		const row = cast(bitmap[y]);
		for(let x = 0; x < width; ++x){
			const off = y * width + (x * 4);
			if(row >>> cast(width - x) & cast(1)){
				rgba[off + 0] = 255 & colour >> 24;
				rgba[off + 1] = 255 & colour >> 16;
				rgba[off + 2] = 255 & colour >> 8;
				rgba[off + 3] = 255 & colour;
			}
		}
	}
	return rgba;
