 ...
 *
 * @param {String|Symbol} [valueKey="innerText"]
 *    Name of property from which to derive definitions from.
 *
 * @param {Function|RegExp} [filter=/^\s+|[\s:]+$/g]
 ...
 * @return {Object}
 */
export function buildDict(dl, valueKey = "innerText", filter = /^\s+|[\s:]+$/g){
	const output = {};
	...
	let keys = [], value, haveValue;
	for(const node of [...dl.children].map(node => {
		switch(node.tagName){
			case "DT": case "DD": return [node];
			case "DIV": return [...node.children].filter(({tagName}) =>
				"DT" === tagName || "DD" === tagName);
		}
	}).filter(Boolean).flat())
		switch(node.tagName){
			case "DT":
				if(haveValue) keys = [];
				haveValue = false;
				keys.push(filter(node[valueKey]));
				break;
