...
/**
 * Strip whitespace surrounding a tagged template literal.
 *
 * @todo Add unit tests and document this with an example.
 * @return {String}
 */
function deindentBlock(...args){
	const depthTable = [];
	let maxDepth = Number.NEGATIVE_INFINITY;
	let minDepth = Number.POSITIVE_INFINITY;
	
	// Normalise newlines and strip leading or trailing blank lines
	const chunk = String.raw.call(this, ...args)
		.replace(/\r(\n?)/g, "$1")
		.replace(/^(?:[ \t]*\n)+|(?:\n[ \t]*)+$/g, "");
