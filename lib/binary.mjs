...
export function utf8Encode(bytes, opts = {}){
	const results = [];
	const {length} = bytes;
	for(let b1, b2, b3, b4, code, size, i = 0; i < length; ++i){
		b1   = bytes[i];
		code = 0xFFFD;
		size = 1;
		...
		// Restrict codepoints to their minimum byte-lengths
		if(!opts.allowOverlong && size > 1 && !(
			code >= [,, 0x080, 0x0800, 0x010000][size] &&
			code <= [,, 0x7FF, 0xFFFF, 0x10FFFF][size]
		)) results.push(...new Array(size).fill(0xFFFD));
