...
/**
 * Convert bytes to 16-bit/2-byte values.
 *
 * @example bytesToWords([0xFF, 0xBB]) == [0xFFBB];
 * @param {Number[]} bytes
 * @return {Number[]}
 */
...
/**
 * Convert bytes to 32-bit/4-byte values.
 *
 * @example bytesToDWords([0xAA, 0xBB, 0xCC, 0xDD]) == [0xAABBCCDD];
 * @param {Number[]} bytes
 * @return {Number[]}
 */
export function bytesToDWords(bytes){
	const words = [], {length} = bytes;
...
/**
 * Convert bytes to 64-bit/8-byte values.
 *
 * @example bytesToQWords([17,34,51,68,85,102,119,136]) == [0x1122334455667788n];
 * @param {Number[]} bytes
 * @return {BigInt[]}
 */
export function bytesToQWords(bytes){
	const words = [], {length} = bytes;
	for(let i = 0; i < length; words.push(
