/**
 * A list of integers that represent individual bytes.
 * @typedef {Number[]|Uint8Array|Uint8ClampedArray} Bytes
 */
 ...
 *
 * @example ascii85Decode("5sdp") == [65, 66, 67];
 * @param {String} input
 * @return {Uint8Array}
 */
export function ascii85Decode(input){
	if((input = input.trim()).startsWith("<~"))
		input = input.slice(2);
	const {length} = input, bytes = [];
	let i = 0, n = 0, w = 0, code;
	main: for(; i < length; ++i)
		switch(code = input.charCodeAt(i)){
			// Ignore whitespace (sans VT)
			case 0x09: // Tab
			case 0x0A: // Line feed
			case 0x0C: // Form feed
			...
			// Tilde: Stop processing
			case 0x7E:
				break main;
			
			// `z`: Add 4 null-bytes
			case 0x7A:
				if(n) throw new SyntaxError("Unexpected `z` shorthand");
				bytes.push(0, 0, 0, 0);
				break;
			...
			default:
				code -= 33;
				if(code > 84)
					throw new SyntaxError(`Unexpected character "${input[i]}"`);
				w = w * 85 + code;
				if(4 === n++)
					bytes.push(
						255 & w >> 24,
	...
	if(i = n){
		while(i++ <= 4) w = w * 85 + 84;
		n > 1 && bytes.push(255 & w >> 24);
		n > 2 && bytes.push(255 & w >> 16);
		n > 3 && bytes.push(255 & w >> 8);
	}
...
/**
 * Encode data using Adobe base85 encoding.
 *
 * @example ascii85Encode([65, 66, 67]) == "5sdp";
 * @param {Bytes} bytes
 * @return {String}
 */
export function ascii85Encode(bytes){
	...
	if(length % 4)
		output = output.slice(0, -(4 - length % 4));
	return output;
}
 ...
 * @param {Number|Number[]|Int8Array} input
 * @return {Uint8Array}
 */
 ...
 * @param {Number|Number[]|Int16Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @param {Number|Number[]|Int32Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @param {BigInt|BigInt[]|BigInt64Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @return {Uint8Array} Raw PNG data
...
export function rgba(r, g, b, a){
	...
	return Uint8Array.from((IHDR + IDAT + data + crc1 + crc2 + "\0".repeat(4) + "IEND\xAEB`\x82")
 ...
 * @param {Number|Number[]|Uint16Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @param {Number|Number[]|Uint32Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @param {BigInt|BigInt[]|BigUint64Array} input
 * @param {Boolean} [littleEndian=false]
 * @return {Uint8Array}
 */
 ...
 * @property {Bytes}    payload - Payload data, expressed in bytes
 * @property {Bytes}    trailer - Excess bytes found after payload
 */
