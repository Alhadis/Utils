 ...
 * @description
 *    Settings controlling how {@link escapeShell} transforms its input.
 *
...
/**
 * How {@link escapeShell} handles newlines (CRLF or LF) when escaping input.
 *
 * @typedef {"collapse"|"error"|"escape"|"ignore"|"quote"|"strip"} NewlineEscapeStrategy
 * @type {"collapse"} - Merge contiguous newlines into a single space character.
 * @type {"error"}    - Throw a {@link TypeError}.
 * @type {"escape"}   - Insert a backslash before each newline.
 * @type {"ignore"}   - Keep newlines as they are (default).
 * @type {"quote"}    - Enclose contiguous newlines with single-quotes.
 * @type {"strip"}    - Remove newlines entirely.
 */

/**
 * How {@link escapeShell} handles null-bytes (U+0000) when escaping input.
 *
 * @typedef {"error"|"escape"|"ignore"|"strip"} NullByteEscapeStrategy
 * @type {"error"}  - Throw a {@link TypeError}.
 * @type {"escape"} - Insert a backslash before each null-byte.
 * @type {"ignore"} - Keep null-bytes as they are (default).
 * @type {"strip"}  - Remove null-bytes entirely.
 */
 ...
 * @return {{type:function, value:*, delimiter:?string, name:?string}}
 */
