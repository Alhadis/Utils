 ...
 *
 * @example camelise("end_of_line") == "endOfLine";
 * @example camelise("syntax-error", {pascalCase: true}) == "SyntaxError";
 * @example camelise("HTML-files", {preserveCaps: true}) == "HTMLFiles";
 * @example camelise("HTML-files", {preserveCaps: "except-first"}) == "htmlFiles";
 *
 * @param {String|String[]} input - An identifier string, or array of identifier segments
 * @param {Object} [options={}] - Settings for controlling case conversion
 * @param {Boolean} [options.pascalCase=false] - Capitalise the initial segment
 * @param {Boolean|"except-first"} [options.preserveCaps=false]
 *    Preserve casing of entirely uppercase segments. The special
 *    value "except-first" exempts initial segments from this treatment.
 * @return {String}
 */
export function camelise(input, options = {}){
	input = Array.isArray(input) ? input.join(" ") : String(input);
	
	const {pascalCase, preserveCaps} = options;
	const {length} = input;
	let result   = "";
	let word     = "";
	let caps     = 0;
	...
	for(let i = 0; i <= length; ++i){
		...
		if(code > 0x60 && code < 0x7B)
			word += !word && (segments || pascalCase) || digits && digits === word.length
				? input[i].toUpperCase()
				: input[i];
		...
		// Other
			...
			if(word)
				result += preserveCaps
				&& (segments || !segments && "except-first" !== preserveCaps)
				&& caps === word.length - digits
					? word.toUpperCase()
					: word;
			word = "";
			caps = digits = 0;
...
/**
 * Convert a camelCased string to an alternative representation.
 *
 * @example decamelise("getURLOfPage")     == "get-url-of-page";
 * @example decamelise("3DModelFile", "_") == "3d_model_file";
 * @example decamelise("<tagName>")        == "<tag-name>";
 * @example decamelise("<tagName>", true)  == "<tagName>";
 *
 * @param {String}  input                - String containing a camelCased identifier
 * @param {String}  [separator="-"]      - String inserted between decamelised segments
 * @param {Boolean} [strict=false]       - Limit conversion to well-formed camelCase only
 * @param {Boolean} [preserveCase=false] - Retain the original casing of segments
 * @return {String}
 */
export function decamelise(input, separator = "-", strict = false, preserveCase = false){
	input = String(input);
	if("boolean" === typeof separator)
		[separator, strict, preserveCase] = ["-", separator, strict];
	
	let result = "", caps = "";
	const {length} = input;
	for(let foundJunk = false, i = 0; i < length; ++i){
		const code = input.charCodeAt(i);
		
		// Uppercase
		if(code > 0x40 && code < 0x5B){
			if(!caps && result && !foundJunk) result += separator;
			caps += preserveCase ? input[i] : input[i].toLowerCase();
		}
		
		// Lowercase
		else if(code > 0x60 && code < 0x7B){
			if(caps.length > 1 + !!result){
				result += caps.slice(0, -1) + separator;
				caps = caps.slice(-1);
			}
		...
		// Digit
			...
			if(strict && !i) return input;
			if(!result || caps) caps += input[i];
			else result += input[i];
		}
...
/**
 * Retrieve the longest common prefix of a set of strings.
 *
 * @see {@link findBasePath}
 * @example findPrefix(["git-config", "git"]) === "git";
...
export function findPrefix(strings){
	...
	if(numStrings < 2)
		return numStrings ? strings[0] : "";
	let prefix = "";
	const maxLength = Math.max(...strings.map(x => x.length));
	X: for(let x = 0; x < maxLength; ++x){
		const char = strings[0][x];
		for(let y = 1; y < numStrings; ++y)
			if(char !== strings[y][x]) break X;
		prefix += char;
	}
	return prefix;
}
...
/**
 * Iterate through chunks of text delineated by a separator string.
 *
 * @example for(const line of strtok(file, "\n")){ … }
 * @param {String} input - String to tokenise
 * @param {String} [sep=" "] - Separator string
 * @param {Boolean} [inclusive=false] - Include separator in chunked text
 * @yields {Token}
 */
export function* strtok(input, sep = " ", inclusive = false){
	...
	do{
		next = input.indexOf(sep, prev);
		yield Object.defineProperty({number, offset: prev, text: -1 === next
			? input.slice(prev)
			: input.slice(prev, next + (inclusive ? sepSize : 0)),
...
/**
 * A tokenised chunk of text returned from {@link strtok}.
 * @typedef {Object} Token
 * @property {Number} number - Index of token, numbered from zero
 * @property {Number} offset - Location of token within input
 * @property {String|Uint8Array} text - Token contents
 */
 ...
 *
 * @example <caption>Alternating brackets and quotation styles</caption>
 * swap(`"A"'B'\`C\``, `"'\``) == "`A`\"B\"'C'";
 * swap("[[(A)]]", ["[]", "()"] == "(([A]))";
 * swap("$(a `b`)", [["$(", ")"], "``"]) == "`a $(b)`";
 *
 ...
 * let decryption = swap(emailAddress, alphabet, 13);
 * decryption     = swap(decryption, alphabet.toLowerCase(), 13);
 * 
 * @param {String} input
 ...
 * @return {String}
 */
export function swap(input, list, steps = 1){
	steps |= 0;
	input = [...String(input)];
