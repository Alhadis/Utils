...
async function exec(cmd, args = [], inputData = null, outputPath = "", encoding = "utf8"){
	...
	const proc = childProcess.spawn(cmd, args, {stdio});
	let stdout = "";
	let stderr = "";
	if(null !== inputData){
...
/**
 * Execute a string as a raw shell-command.
 *
 * Works best when aliased to `$` and used with tagged template literals.
 * Remember not to pass untrusted or unsanitised user input as arguments!
 *
 * @example <caption>Silly-looking examples</captions>
 *   import {execRaw as $} from "./lib/node.js";
 *   const checksums = await $ `git log --oneline | cut -d' ' -f1`;
 *   const forty     = await $ `printf %s "${checksums}" | wc -l`;
 *
 * @example <caption>Parse Unicode's character database</caption>
 *   // This actually fails with "Error: stdout maxBuffer exceeded", but w/e
 ...
 * @param {...String} values
 * @return {Promise}
 *   Resolves with the last command's standard output.
 */
async function execRaw(first, ...values){
	const source = Array.isArray(first) && Array.isArray(first.raw)
		? first.raw.map((string, index) => string + (values[index] || "")).join("")
		: [first, ...values].join(" ");
	return new Promise((resolve, reject) =>
		childProcess.exec(source, (error, stdout, stderr) =>
