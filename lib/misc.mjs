 ...
 *
 * The `sizes` array is repeated indefinitely until the entirety of the input
 * has been allocated. If `input` lacks enough elements to precisely fill the
 * last partitioned section, the returned list will end with a sparse array.
 *
 * @example partition(["A", "B", "C"], [2, 1]) == [["A", "B"], ["C"]];
 * @example partition("AAAABB", [4]) == [["A", "A", "A", "A"], ["B", "B",,,]];
 * @param {String|Iterable} input
 * @param {Number|Number[]} [sizes=[1]]
 * @return {Array[]}
 */
export function partition(input, sizes = [1]){
	input = "string" === typeof input ? input.split("") : input;
