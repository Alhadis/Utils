 ...
 *
 * @example sip("./bin/run", 20) == "#!/usr/bin/env node\n";
 * @param {String}  file         - Path to file
 * @param {Number}  [length=80]  - Maximum number of bytes to read
 * @param {Number}  [offset=0]   - Offset to begin reading from
 * @param {Boolean} [raw=false]  - Return a raw byte-array instead of a string
 * @return {Promise<(String|Uint8Array)>}
 *    Resolves with either a UTF8-encoded string or an array of
 *    bytes, depending on the value of the `raw` parameter.
 */
export async function sip(file, length = 80, offset = 0, raw = false){
	if(!file || length < 1)
		return raw ? new Uint8Array(0) : "";
	
	const {openSync, read} = await import("fs");
	const fd = openSync(file, "r");
	return new Promise((resolve, reject) => {
		const buffer = Buffer.alloc(length);
		read(fd, buffer, 0, length, offset, (error, bytesRead) => {
			if(error) return reject(error);
			const end = Math.min(bytesRead, length);
			return resolve(raw
				? Uint8Array.from(buffer.slice(0, end))
				: buffer.toString("utf8", 0, end));
		});
	});
}
