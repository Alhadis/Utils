...
/**
 * Recurse through a directory and return each file found.
 *
 * @example <caption>Find all JavaScript and JSON files in current directory</caption>
 *    await findFile(".", {match: /\.(js|json)$/i}) => Map {
 *       "./package.json" => Stats {dev: 16777220, mode: 33188, nlink: 1, uid: …},
 *       "./lib/index.js" => Stats {dev: 16777220, mode: 33188, nlink: 1, uid: …},
 *       "./lib/shell.js" => Stats {dev: 16777220, mode: 33188, nlink: 1, uid: …},
 *    };
 * @param {String} path - Root directory to start searching from.
 * @param {FileSearchOptions} [options={}] - Optional settings to refine search results.
 * @param {?Number} [currentDepth=null] - Zero-based index of current recursion level (internal-use only).
 * @return {Promise.<Map<String, fs.Stats>>}
 */
export async function findFiles(path, options = {}, currentDepth = null){
	const {readdir, lstat} = await import("fs");
	const {resolve, join}  = await import("path");
	
	// Normalise arguments on first-run only (skipped during recursion)
	let {match, ignore, maxDepth} = options;
	if(null === currentDepth){
		currentDepth = -1;
		const fn = x => "function" === typeof x ? x : x instanceof RegExp ? s => x.test(s) : null;
		path     = resolve(path || process.cwd());
		match    = fn(match)    || (() => true);
		ignore   = fn(ignore)   || (path => /(?:^|[\\/])(?:\.git|node_modules)$/i.test(path));
		maxDepth = Number.isNaN(+maxDepth) ? -1 : ~~maxDepth;
		options  = {match, ignore, maxDepth};
	}
	...
	)].filter(([path, stats]) => match(path, stats) && !ignore(path, stats)));
	const searches = [];
	if(maxDepth < 0 || ++currentDepth < maxDepth)
		for(const [path, stats] of files)
			stats.isDirectory() && searches.push(findFiles(path, options, currentDepth));
 ...
 * @summary Optional settings used by {@link findFiles} for narrowing down search results.
 *
 * @property {FileSearchFilter} [match = () => true]
 *    Pattern or predicate function to restrict matching search results to.
 *
 * @property {FileSearchFilter} [ignore = /(?:^|[\\/])(?:\.git|node_modules)$/i]
 *    Pattern or predicate function to filter out unwanted files or directories.
 *    By default, `.git` and `node_modules` directories will be glossed over.
 *
 * @property {Number} [maxDepth=-1]
 *    Maximum directory levels to descend, with 0 limiting the search to the
 *    current directory, and negative values causing no limit to be imposed.
 */

/**
 * @typedef {RegExp|FileSearchPredicate} FileSearchFilter
 * @description
 *    Regular expression tested against a file's resolved path, or a predicate function
 *    called with a file's resolved path and its {@link fs.Stats} as arguments, respectively.
 */

/**
 * @callback FileSearchPredicate
 * @param  {String}   path
...
export async function rmrf(paths = [], ignoreErrors = false){
	...
	const exec = async (cmd, ...argv) => new Promise((resolve, reject) => {
		...
		subproc.on("close", code => !ignoreErrors && code
			? reject(`${cmd} ${argv.join(" ")} exited with ${code}`)
			: resolve());
	});
